...
| Preview    | Vercel Preview 独立数据库或 Schema                         | Vercel Preview         | Eve Runtime 和模型凭据                | 尚未配置     | 待真实环境验证   |
| Production | 托管 PostgreSQL，启用 PITR 和连接池                        | Vercel Production      | Eve Runtime、模型凭据和 Sandbox 策略  | 尚未配置     | 待真实环境验证   |
